...
class RealTimePred:
    def __init__(self):
        self.logs = dict(name=[],role=[],current_time=[], lat=[], long=[])
    ...
    def saveLogs_redis(self):
        ...
        long_list = dataframe['long'].tolist()
        
        encoded_data = []
        for name, role, ctime, lat, long in zip(name_list, role_list, ctime_list, lat_list, long_list):
            if name != 'Unknown':
                # Format: name@role@timestamp@lat@long
                concat_string = f"{name}@{role}@{ctime}@{lat}@{long}"
    ...
    def face_prediction(self,test_image, dataframe,feature_column,
                            name_role=['Name','Role'],thresh=0.5, lat=0.0, long=0.0):
